...
Q1=Q;
for i=3:(length(P))
    Z(i)=Q(i)-Q(i-1);  %// Stationnariser par la première différence
    %Z(i)=Q(i)-2*Q(i-1)+Q(i-2);
    Zp(i)=P(i)-P(i-1);
    %Zp(i)=P(i)-2*P(i-1)+P(i-2);
end
...
yh(i)=yh(i)+Q1(i-1);  %// Destationnariser 1 différence
%yh(i)=yh(i)+2*Q1(i-1)+Q(i-2);  %// Destationnariser 2 différence
